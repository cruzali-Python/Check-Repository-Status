...
for repo_path in os.listdir(carpeta_repositorios):
    ...
    try:
        ...
        if 'Changes not staged for commit' in repo_status or 'Your branch is ahead of' in repo_status:
            print(
                f"{Fore.LIGHTYELLOW_EX}Proyecto '{repo_path}': Pendiente de commit y/o push{Style.RESET_ALL}")
        else:
            print(
                f"{Fore.LIGHTGREEN_EX}Proyecto '{repo_path}': Actualizado{Style.RESET_ALL}")
        ...
        print(
            f"{Fore.LIGHTRED_EX}Error al abrir el repositorio '{repo_path}': {str(e)}{Style.RESET_ALL}")
